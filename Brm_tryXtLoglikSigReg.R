...
transformed parameters {
  ...
  bwas[1] = bws[1];
  for(i in 2:pn)
  bwas[i] = psis*bwas[i-1] + bws[i]*tetas;
  

  lprior += std_normal_lpdf(bw);
  lprior += uniform_lpdf(psi | 0.01, .99);
  lprior += std_normal_lpdf(bws);
  lprior += uniform_lpdf(psis | 0.01, .99);
  lprior += uniform_lpdf(gam | Bd[1], Bd[2]);
...
model {
  ...
  if (!prior_only) {
    ...
    sigma += Intercept_sigma + Xs_sigma * bs_sigma + Zs_sigma_1_1 * s_sigma_1_1 + Wn * bwas;
    sigma = exp(sigma);
    for (n in 1:N) {
...
//    sigma = exp(sigma);
 ...
 #------------------------------------------------------------------------------
  ...
  PostAnalyStan <- function(fit,  Bx=bs2, tau0, df0=10, smoothT = F){
    ...
    Gam <- (PostMat[, grepl("bwa", colnames(PostMat))])[,1:10];  # 
    ...
    for(i in 1:nrow(Fw)){
     ...
     FwSmthx[i,] <- Betafunc(x,met[idf]) #2.5*((Betafunc(x,met[idf]) - mean(Betafunc(x,met[idf])))*c(1,2,2, 2)[idf] + qnorm(tau)*((Betafunc(x,met[idf]) - mean(Betafunc(x,met[idf])))*c(1,2,2, 2)[idf]))
