...
InitialValues <- function(Y, W, M, Z, df0 = 3, pn, Gx, a) {
  ...
  WsM <- NULL
  for (i in 1:nrow(W)) {
    WsM <- rbind(WsM, smooth.spline(a, W[i, ])$y)
  }
  ...
  for (j in 1:Tp$G) {
    Res$Sigk.w[j, ] <- c(Tp$variance[, , j])
  }
  for (j in (Tp$G + 1):Res$Kw) {
    Res$Sigk.w[j, ] <- c(as.positive.definite(.5 * diag(pn)))
  }
  ...
  WsM <- NULL
  for (i in 1:nrow(W)) {
    WsM <- rbind(WsM, smooth.spline(a, WsmB[i, ])$y)
  }
  W_i <- crossprod(t(WsM), bs2) / length(a) # (n,k) matrix
  Ui <- W_i
  Mod1 <- Mclust(Ui, G = 2)
  ...
  for (j in 1:Tp$G) {
    Res$Sigk.x[j, ] <- c(Tp$variance[, , j])
  }
  for (j in (Tp$G + 1):Res$Kx) {
    Res$Sigk.x[j, ] <- c(.5 * diag(pn))
  }
  ...
  for (j in 1:Tp$G) {
    Res$Sigk.m[j, ] <- c(Tp$variance[, , j])
  }
  for (j in (Tp$G + 1):Res$Km) {
    Res$Sigk.m[j, ] <- c(as.positive.definite(.5 * diag(pn)))
  }
...
InitialValuesQR2 <- function(Y, W, M, Z, df0 = 3, pn, Gx, a, tau0) {
  ...
  Tp0 <- optim(c(.2, -.1, rnorm(n = ncol(Wn))), LogPlostGalLikBet,
    method = method, ei = c(ei), X = Wn, tau0 = tau0, control = list("fnscale" = -1), hessian = T, lower = c(.005, 0.95 * min(bnd), rep(-7, ncol(Wn))),
    upper = c(30, 0.95 * max(bnd), rep(7, ncol(Wn)))
  )
